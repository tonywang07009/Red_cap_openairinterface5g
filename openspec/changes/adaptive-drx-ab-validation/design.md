...
- [Baseline] Arm A SHALL pre-apply `drx-320-10` as policy version 1 once and retain it for all 300 scored arrivals. On fresh unconfigured state, Arm B SHALL first commit the same profile under reserved local bootstrap version 0 so FlexRIC request IDs 1 and above remain monotonic.
...
- [Prediction] The xApp SHALL calculate `mu`, `sigma`, and `mu +/- 3 sigma` from exactly 30 committed samples. It SHALL also report median, p95, min, and max and select a conservative fallback before E2 when either bound is unreliable. Because standard RAN Parameter 1 carries only the long cycle, the dApp SHALL validate the resulting legal profile and live UE/reconfiguration safety state, not re-derive Python-local statistics.
...
- [Window lifecycle] The runner/xApp SHALL commit a policy only after the dApp acceptance, gNB applied-state marker, and expected RRC completion evidence are present. It SHALL clear the 30-sample window only after that acknowledgement; rejected or timed-out requests persist the trace hash, row range, exact intervals, and reject reason.
