...
- Do not run 56 UE RFsim in this first static implementation.
...
- [Staged gates] Use Gate A-E: SDK unit, SWIG, E3 loopback, small RFsim, and 56 UE / 5 MHz BWP stress. This prevents a 56 UE failure from hiding whether the problem is SDK logic, E3 binding, or RFsim scale.
...
- [Risk] 5 MHz BWP can fail because PUCCH resources are constrained after the profile maps to a small PRB budget. -> Mitigation: treat 56 UE / 5 MHz as a stress gate and record scheduler/config failure separately from SDK failure.
