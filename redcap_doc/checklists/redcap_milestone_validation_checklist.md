...
| Runtime evidence links point to temporary logs under `test_log/` or promoted evidence under `redcap_library/` | Project validation rule; no direct 3GPP clause |
...
| 2026-05-25 cleanup supersedes the inventory-only retention rule: reusable evidence is promoted to `redcap_library/`, temporary process logs use `test_log/`, and legacy `test_logs/` is removed | Repository hygiene gate; no direct 3GPP clause |
