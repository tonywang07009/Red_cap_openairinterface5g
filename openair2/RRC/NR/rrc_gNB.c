...
int rrc_gNB_generate_pcch_msg(sctp_assoc_t assoc_id, const NR_SIB1_t *sib1, uint32_t tmsi, uint8_t paging_drx)
{
  ...
  const NR_PCCH_Config_t *pcch_config = &sib1->servingCellConfigCommon->downlinkConfigCommon.pcch_Config;
  const NR_SetupRelease_PDCCH_ConfigCommon_t *pdcch_config =
      sib1->servingCellConfigCommon->downlinkConfigCommon.initialDownlinkBWP.pdcch_ConfigCommon;
  long paging_search_space = NR_RRC_PAGING_SEARCH_SPACE_UNCONFIGURED;
  if (pdcch_config != NULL && pdcch_config->present == NR_SetupRelease_PDCCH_ConfigCommon_PR_setup
      && pdcch_config->choice.setup != NULL && pdcch_config->choice.setup->pagingSearchSpace != NULL)
    paging_search_space = *pdcch_config->choice.setup->pagingSearchSpace;

  nr_rrc_paging_parameters_t paging_parameters = {0};
  if (nr_rrc_get_paging_parameters(pcch_config, paging_search_space, paging_drx, &paging_parameters)
      != NR_RRC_PAGING_PARAMETERS_OK) {
    LOG_E(RRC, "[gNB %ld] In rrc_gNB_generate_pcch_msg: invalid PCCH paging parameters\n", instance);
