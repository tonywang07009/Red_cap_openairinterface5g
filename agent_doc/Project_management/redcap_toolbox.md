...
- [Core RedCap path]: `symdex -> rtk rg/read -> targeted build/test/log marker -> concise report`.

## Three-Layer Tool Route

| Layer | Purpose | Default Tools | Stop Condition |
|---|---|---|---|
| [Layer 1: Navigation] | find OAI C code path, symbol, caller/callee candidates, and existing markers | `symdex`, targeted `rtk rg` | target files/functions and markers are known |
...
|---|---|---|---|
| C symbol / file outline / protocol path | [symdex] | `rtk rg -n` | Use `symdex search`, `symdex text`, and `symdex outline` before broad source reads. |
| Function callers/callees | [symdex callers/callees] | `symdex outline` + targeted `rtk rg -n` | Caller/callee coverage can be incomplete for some C paths. |
| Project docs / local specs / logs | [filesystem MCP] or targeted shell reads | `rtk sed`, `rtk rg` | Read only the active context pack. |
| Build / unit test / runtime | `rtk` shell commands | raw shell only for shell builtins or unsupported syntax | Keep logs under `test_log/compiler_logs/` or `test_log/build_logs/`. |
| PDF conversion | existing MinerU cache | [MinerU MCP] | Check `redcap_doc/mineru_markdown/scan_manifest.md` before parsing PDFs. |
| Office / HTML / simple PDF conversion | [markitdown MCP] | MinerU or direct Markdown read | Avoid for Unicode-heavy files unless a small probe succeeds. |
| Cloud docs / report export | [Google Drive connector] | local Markdown/PDF artifacts | Use only when the task explicitly needs Drive/Docs/Sheets/Slides. |
| PR / CI / GitHub review | [GitHub plugin/skill] | local `git` and `gh` if approved | Not part of daily protocol debugging unless the task is GitHub-specific. |
...
```bash
rtk rg -n "<symbol-or-marker>" openair1 openair2 openair3 common executables ci-scripts redcap_interface
```
