...
- [MUST] Use `Apps_dev/dapp_dev_need/libe3/` for E3 role, transport, encoding, and SWIG binding expectations.
- [MUST] Use `Apps_dev/dapp_dev_need/E3Controller/` for I/Q pipeline, per-slot pipeline, `--num-prbs`, and timing-log expectations.
- [MUST] Use `Apps_dev/dapp_dev_need/dApp-library/` for I/Q sample, PRB control, and visualization examples.
- [MUST] Use `Apps_dev/dapp_dev_need/dApp-openairinterface5g/` only as a targeted implementation reference, not as a source tree to copy.
- [MUST] Use `Apps_dev/xapp_dev_need/` and `openair2/E2AP/flexric/` for xApp design comparison.
...
- Gate E preflight evidence:
  - Supported 56 UE RFsim overlay generated by `redcap_interface/bash_library/generate_mmtc_overlay.sh 56 test_log/runtime_configs/<run-id>_overlay.yml`.
  - RFsim test topology is based on `ci-scripts/yaml_files/5g_rfsimulator_flexric_redcap/docker-compose.yml` plus a run-specific overlay in `test_log/runtime_configs/`.
  - CN/AMF topology is based on the repository-owned `oai-cn5g/docker-compose.yaml`; compose config lists `oai-amf`, `mysql`, `oai-smf`, and `oai-upf`.
  - CN DB overlay generated by `redcap_interface/generate_mmtc_cn_db_overlay.sh 56`.
  - Preflight checker PASS: `gate_e_64ue_stage_check.py`.
  - The checker confirms UE1..UE56 compose services, UE1..UE56 RedCap UICC configs, UE56 IMSI in the generated SQL overlay, 5 MHz/12 PRB first-stage profile, and 51 PRB 20 MHz proxy profile `[Needs Verification]`.
- Gate E first32 runtime attempt:
  - This retained historical attempt used the former 64 UE overlay; it is not the current supported service ceiling.
