...
export MMTC_RRC_INACTIVE_GATE2_RESUME_TRIGGER="${MMTC_RRC_INACTIVE_GATE2_RESUME_TRIGGER:-1}"
...
note=Policy-level SDT thresholds are recorded in the YAML matrix and must be applied through the RedCap control contract before measured runs.
EOF

echo "[SDT] manifest: ${LOG_DIR}/run_manifest.txt"
echo "[SDT] config: ${CONFIG_FILE}"
echo "[SDT] services: ${SERVICES}"

if [[ "${RUN_MODE}" == "--dry-run" ]]; then
  echo "[SDT] dry-run only. Use --run to start docker compose."
  exit 0
fi
