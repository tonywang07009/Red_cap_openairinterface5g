...
if [ "${USE_LOCAL_OAI_IMAGES}" = "1" ] && [ "${REBUILD_LOCAL_OAI_IMAGES}" = "1" ]; then
  ...
  "${REPO_ROOT}/redcap_interface/redcap_rebuild_local_oai_images.sh"
fi
...
if [ "${E2_AGENT_MODE}" != "enabled" ]; then
  ...
  python3 "${REPO_ROOT}/ci-scripts/redcap_prepare_runtime_config.py" "${prepare_args[@]}"
...
python3 "${REPO_ROOT}/ci-scripts/redcap_runtime_summary.py" "${summary_args[@]}"
...
if grep -Eq 'Cannot allocate all required PUCCH resources for max number of [0-9]+ UEs in BWP with [0-9]+ PRBs' "${RUN_LOG}"; then
  ...
  echo "       bash redcap_interface/redcap_rebuild_local_oai_images.sh"
  echo "       bash redcap_interface/redcap_inspect_gnb_image.sh"
  echo "       REDCAP_USE_LOCAL_OAI_IMAGES=1 REDCAP_E2_AGENT_MODE=disabled bash redcap_interface/redcap_runtime_host_validation.sh"
fi
