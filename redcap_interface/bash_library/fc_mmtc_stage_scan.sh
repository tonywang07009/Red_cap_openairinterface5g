...
STAGE_PROFILE=${MMTC_STAGE_PROFILE:-}
if [ "${STAGE_PROFILE}" = "core36_pressure" ]; then
  TOTAL_UES_TARGET=${MMTC_TOTAL_UES_TARGET:-56}
  STAGE_LIST_RAW=${MMTC_STAGE_LIST:-"36"}
  UE_START_GAP=${MMTC_UE_START_GAP:-0}
  MMTC_ADAPTIVE_BURST_ON_ZERO_GAP=${MMTC_ADAPTIVE_BURST_ON_ZERO_GAP:-0}
else
  TOTAL_UES_TARGET=${MMTC_TOTAL_UES_TARGET:-50}
  STAGE_LIST_RAW=${MMTC_STAGE_LIST:-"50"}
  UE_START_GAP=${MMTC_UE_START_GAP:-10}
  MMTC_ADAPTIVE_BURST_ON_ZERO_GAP=${MMTC_ADAPTIVE_BURST_ON_ZERO_GAP:-1}
fi
...
MMTC_IPERF_ENABLE_USER_SET=${MMTC_IPERF_ENABLE+x}
MMTC_IPERF_ENABLE=${MMTC_IPERF_ENABLE:-1}
if [ "${STAGE_PROFILE}" = "core36_pressure" ] && [ -z "${MMTC_IPERF_ENABLE_USER_SET}" ]; then
  MMTC_IPERF_ENABLE=0
fi
...
MMTC_DAPP_PRIORITY_UES=${MMTC_DAPP_PRIORITY_UES:-}
MMTC_DAPP_STOP_NON_PRIORITY=${MMTC_DAPP_STOP_NON_PRIORITY:-0}
OAI_REDCAP_DAPP_RA_RETRY_PRIORITY_USER_SET=${OAI_REDCAP_DAPP_RA_RETRY_PRIORITY+x}
OAI_REDCAP_DAPP_RA_RETRY_PRIORITY=${OAI_REDCAP_DAPP_RA_RETRY_PRIORITY:-0}
if [ "${STAGE_PROFILE}" = "core36_pressure" ] \
   && [ "${START_XAPP}" = "1" ] \
   && [ -n "${MMTC_DAPP_PRIORITY_UES}" ] \
   && [ -z "${OAI_REDCAP_DAPP_RA_RETRY_PRIORITY_USER_SET}" ]; then
...

{
  ...
  echo "# stages=${STAGES[*]}"
  echo "# stage_profile=${STAGE_PROFILE:-manual}"
  ...
  echo "# ue_start_gap=${UE_START_GAP} adaptive_burst_on_zero_gap=${MMTC_ADAPTIVE_BURST_ON_ZERO_GAP}"
  echo "# dapp_priority_ues=${MMTC_DAPP_PRIORITY_UES:-none} dapp_stop_non_priority=${MMTC_DAPP_STOP_NON_PRIORITY}"
  echo "# dapp_ra_retry_priority=${OAI_REDCAP_DAPP_RA_RETRY_PRIORITY}"
...
for stage in "${STAGES[@]}"; do
  ...
  env \
    ...
    MMTC_ADAPTIVE_BURST_ON_ZERO_GAP="${MMTC_ADAPTIVE_BURST_ON_ZERO_GAP}" \
    ...
    MMTC_IPERF_SERVER_IP="${MMTC_IPERF_SERVER_IP}" \
    MMTC_DAPP_PRIORITY_UES="${MMTC_DAPP_PRIORITY_UES}" \
    MMTC_DAPP_STOP_NON_PRIORITY="${MMTC_DAPP_STOP_NON_PRIORITY}" \
    OAI_REDCAP_DAPP_RA_RETRY_PRIORITY="${OAI_REDCAP_DAPP_RA_RETRY_PRIORITY}" \
