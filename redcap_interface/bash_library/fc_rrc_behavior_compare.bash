...
POLICY_HOST_FILE=${RRC_BEHAVIOR_POLICY_HOST_FILE:-"${REPO_ROOT}/redcap_interface/control/redcap_policy_case_a.yaml"}

RUN_EXPERIMENTS=${RRC_BEHAVIOR_RUN_EXPERIMENTS:-1}
RUN_IDLE=${RRC_BEHAVIOR_RUN_IDLE:-1}
RUN_INACTIVE=${RRC_BEHAVIOR_RUN_INACTIVE:-1}

TOTAL_UES=${RRC_BEHAVIOR_TOTAL_UES:-${MMTC_TOTAL_UES:-56}}
SAMPLE_UES_RAW=${RRC_BEHAVIOR_SAMPLE_UES:-${MMTC_ACTIVE_UES:-1}}
...
if [ -z "${FIRST_SAMPLE_UE}" ]; then
  echo "No UE index found in RRC_BEHAVIOR_SAMPLE_UES/MMTC_ACTIVE_UES" >&2
  exit 1
fi
